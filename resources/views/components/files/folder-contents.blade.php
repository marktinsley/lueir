                    ...
                    @foreach($folders as $folder)
                        <div class="bg-white hover:bg-gray-600 hover:text-white flex"
                            ...
                            <div class="flex-none w-12 py-5 pr-6 flex justify-end">
                                <div
                                    x-data="{ initialFill: '{{ \App\Models\Favorite::isFavorite($folder) ? '#ffff00' : '#ffffff' }}', fill: '{{ \App\Models\Favorite::isFavorite($folder) ? '#ffff00' : '#ffffff' }}' }">
                                    <svg class="ml-3 w-5 cursor-pointer" xmlns="http://www.w3.org/2000/svg"
                                         x-bind:fill="fill"
                                         stroke="#000055"
                                         viewBox="0 0 24 24"
                                         onclick="Livewire.emit('addFavorite', '{{ $folder->relativePath() }}')"
